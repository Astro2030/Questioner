...
class QuestionModel:
    ...
    def update_downvotes(self, question_id, Downvote):
        '''Increment votes'''
        query_string = 'update questions set downvotes = %(Downvote)s where id=%(question_id)s;'
        cursor = self.conn.cursor()
        return cursor.execute(query_string,{'question_id':question_id, 'Downvote':Downvote})
